...
begin	
	...
	/* Obtenemos el periodo de inicio para las vacaciones truncas.
	   ...
	   (si no, se elige un periodo futuro y XFALTASVACA queda en 0).
	   Contar PR_VacationDetail por @idvac (no por subquery correlacionada). */

	select line, ControlYear, DateBeginProvision
	into #vacaciones
	from PR_Vacation
	where Person = @person
	  and Company = @company
	  and ControlYear between year(@fechaingreso) and year(@ceasedate)
	  and DateBeginProvision <= @ceasedate

	WHILE EXISTS (SELECT 1 FROM #vacaciones)
	BEGIN

		set @idvac = (select top 1 line from #vacaciones order by ControlYear desc)
		set @cantvac = isnull((
			select count(*)
		...
		if @cantvac = 0
		begin
			set @periodo_inicial = (select DateBeginProvision from #vacaciones where line = @idvac)
			break
		end
