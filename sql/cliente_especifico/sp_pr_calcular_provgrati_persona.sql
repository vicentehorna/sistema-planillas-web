...
CREATE procedure [dbo].[sp_pr_calcular_provgrati_persona]
...
begin	
	...
	--set @fechaactual = case when @ceasedate is null then convert(datetime,left(@period,6)+'30') else @ceasedate end
	set @fechaactual = case when @ceasedate is null then convert(datetime,left(@period,6)+'30') else case when @ceasedate < convert(datetime,convert(varchar(6),@ceasedate,112) + '30') then convert(date,DATEADD(DAY, 30, EOMONTH(@ceasedate, -2))) else convert(date,@ceasedate)  end end
	
		
	set @dia_grati_trunca = case when MONTH (@fechaactual) < 7 then
								case when convert(date,@fechaingreso) > convert(date, left(@period,4) +'0101') then  (case when @fechaingreso > convert(datetime,convert(varchar(6),@fechaingreso,112) + '01') then dbo.f_getDias360(convert(date,convert(varchar(6),dateadd(month,1,@fechaingreso),112) + '01') ,convert(date,@fechaactual) ) else dbo.f_getDias360(convert(date,@fechaingreso) ,convert(date,@fechaactual) ) end ) else dbo.f_getDias360(convert(date, left(@period,4) +'0101') ,convert(date,@fechaactual) ) end 
							else
								case when convert(date,@fechaingreso) > convert(date, left(@period,4) +'0701') then  (case when @fechaingreso > convert(datetime,convert(varchar(6),@fechaingreso,112) + '01') then dbo.f_getDias360(convert(date,convert(varchar(6),dateadd(month,1,@fechaingreso),112) + '01') ,convert(date,@fechaactual) ) else dbo.f_getDias360(convert(date,@fechaingreso) ,convert(date,@fechaactual) ) end ) else dbo.f_getDias360(convert(date, left(@period,4) +'0701') ,convert(date,@fechaactual) ) end 
							end
